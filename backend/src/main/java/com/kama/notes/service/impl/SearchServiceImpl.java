...
public class SearchServiceImpl implements SearchService {
    ...
    public ApiResponse<List<Note>> searchNotes(String keyword, int page, int pageSize){
        try {
            List<NoteDocument> noteDocs = noteEsRepository.searchByKeyword(keyword);
            System.out.println("拿到文档数量: " + noteDocs.size());

            List<Note> noteList = noteDocs.stream().map(doc -> {
                Note note = new Note();
                BeanUtils.copyProperties(doc, note);
                return note;
            }).toList();

            return ApiResponseUtil.success("搜索问题成功", noteList);
        } catch (Exception e) {
